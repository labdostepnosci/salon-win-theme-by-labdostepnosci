...
### Changed
- Przebudowano pierwszy ekran strony głównej na pełnoekranowy slider zdjęć miejsca z miniaturami i podpisami, korzystający z istniejących obrazów Customizera.
- Zmieniono komunikację hero z akcentu winiarskiego na butikowe apartamenty, wnętrza, sztukę, gości hotelowych i kameralne wydarzenia.
- Rozjaśniono paletę motywu oraz sekcję rezerwacji, aby strona była cieplejsza, lżejsza i mniej ponura.
- Ograniczono dominację komunikatów o winie na pierwszej stronie; wino występuje jako element pobytu, kolacji i wydarzeń dla gości.
- Dodano automatyczne przełączanie zdjęć w hero z obsługą miniatur, pauzą przy interakcji i poszanowaniem `prefers-reduced-motion`.
- Doprecyzowano etykiety obrazów w Customizerze, ponieważ wybrane zdjęcia galerii są teraz używane również w sliderze hero.

### Checked
- Sprawdzono składnię JavaScript pliku `assets/js/main.js` lokalnym runtime’em Node.
- Nie wykonano `php -l`, ponieważ PHP nie jest dostępne w tej sesji na ścieżce systemowej ani w zależnościach Codexa.

...
(function () {
  ...
  /* ============================================================
     2B. HERO IMAGE STORY SLIDER
  ============================================================ */
  const initHeroSlider = () => {
    const hero   = qs('#hero');
    const slider = qs('#hero-bg');
    ...
    const label   = qs('#hero-slide-label', hero);
    const title   = qs('#hero-slide-title', hero);
    const caption = qs('#hero-slide-caption', hero);
    ...
    const setCopy = (slide) => {
      ...
      if (caption) caption.textContent = slide.dataset.caption || '';
    };
    ...
    const start = () => {
      ...
      timer = setInterval(() => go(current + 1), 5600);
    };

    thumbs.forEach((thumb, index) => {
      thumb.setAttribute('aria-pressed', String(index === current));
      on(thumb, 'click', () => {
        go(index);
        stop();
        start();
